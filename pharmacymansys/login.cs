                ...
                if (comboBox1.SelectedIndex == 0)
                {
                    ...
                    try
                    {
                        using (OracleConnection conn = new OracleConnection(connectionString))
                        {
                            ...
                            OracleCommand cmd =
                                new OracleCommand("SELECT * FROM ADMIN WHERE ADMIN_USERNAME='" + textBox1.Text + "'",
                                    conn);
                            ...
                            if (r.HasRows)
                            {
                                cmd = new OracleCommand(
                                    "SELECT PASSWORD FROM ADMIN WHERE ADMIN_USERNAME='" + textBox1.Text + "'", conn);
                                r = cmd.ExecuteReader();
                                r.Read();
                ...
                if (comboBox1.SelectedIndex == 1)
                {
                    ...
                    try
                    {
                        using (OracleConnection conn = new OracleConnection(connectionString))
                        {
                            ...
                            OracleCommand cmd =
                                new OracleCommand("SELECT * FROM MANAGER_LOGIN WHERE U_NAME='" + textBox1.Text + "'",
                                    conn);
                            OracleDataReader r = cmd.ExecuteReader();
                            if (r.HasRows)
                            {
                                // MessageBox.Show();
                                cmd = new OracleCommand(
                                    "SELECT PASSWORD FROM MANAGER_LOGIN WHERE U_NAME='" + textBox1.Text + "'", conn);
                                r = cmd.ExecuteReader();
                                r.Read();
                ...
                if (comboBox1.SelectedIndex == 2)
                {
                    ...
                    try
                    {
                        using (OracleConnection conn = new OracleConnection(connectionString))
                        {
                            ...
                            OracleCommand cmd =
                                new OracleCommand("SELECT * FROM STUFF_LOGIN WHERE U_NAME='" + textBox1.Text + "'",
                                    conn);
                            OracleDataReader r = cmd.ExecuteReader();
                            if (r.HasRows)
                            {
                                // MessageBox.Show();
                                cmd = new OracleCommand(
                                    "SELECT PASSWORD FROM STUFF_LOGIN WHERE U_NAME='" + textBox1.Text + "'", conn);
                                r = cmd.ExecuteReader();
                                r.Read();
                                // MessageBox.Show(r.GetString(0));
                                if (r.GetValue(0).ToString() == textBox2.Text)
