            ...
            else
            {
                try
                {
                    using (OracleConnection conn = new OracleConnection(connectionString))
                    {
                        ...
                        OracleCommand cmd = new OracleCommand("SELECT * FROM ADMIN WHERE ADMIN_USERNAME='" + name + "'",
                            conn);
                        OracleDataReader r = cmd.ExecuteReader();
                        if (r.HasRows)
                        {
                            // MessageBox.Show();
                            cmd = new OracleCommand("SELECT PASSWORD FROM ADMIN WHERE ADMIN_USERNAME='" + name + "'",
                                conn);
                            r = cmd.ExecuteReader();
                            r.Read();
                            ...
                            if (r.GetValue(0).ToString() == textBox3.Text)
                            {
                                ...
                                try
                                {
                                    ...
                                    cmd = new OracleCommand(
                                        "UPDATE ADMIN SET PASSWORD='" + textBox4.Text + "' WHERE ADMIN_USERNAME='" +
                                        name + "'", conn);
                                    cmd.ExecuteNonQuery();
                                    conn.Close();
                        ...
                        {
                            MessageBox.Show("Invalid User Name Or Password", "Access Denied", MessageBoxButtons.OK,
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                using (OracleConnection conn = new OracleConnection(connectionString))
                {
                    ...
                    OracleCommand cmd =
                        new OracleCommand(
                            "SELECT * FROM ADMIN WHERE ADMIN_USERNAME='" + name + "' AND PASSWORD='" + textBox2.Text +
                            "'", conn);
                    OracleDataReader r = cmd.ExecuteReader();
                    ...
                    if (r.HasRows)
                    {
                        try
                        {
                            ...
                            /* OracleCommand*/
                            cmd = new OracleCommand(
                                "UPDATE ADMIN SET ADMIN_USERNAME='" + textBox1.Text + "' WHERE ADMIN_USERNAME='" +
                                name + "'", conn);
                            cmd.ExecuteNonQuery();
                            conn.Close();

                            r.Close();
                            MessageBox.Show("Your user name has been changed to " + textBox1.Text + "", "Success",
